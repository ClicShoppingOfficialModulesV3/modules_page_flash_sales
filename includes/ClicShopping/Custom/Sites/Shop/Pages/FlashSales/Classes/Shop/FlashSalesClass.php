<?php
  ...
  namespace ClicShopping\Custom\Sites\Shop\Pages\FlashSales\Classes\Shop;

  use ClicShopping\OM\Registry;

  class FlashSalesClass
  {

// Sets the status of a special product
    private static function setFlashSalesStatus($specials_id, $status)
    {
...
// Auto activate scheduled products on special
    public static function scheduledFlashSales()
    {
      $CLICSHOPPING_Db = Registry::get('Db');
      ...
      if ($Qspecials->fetch() !== false) {
        do {
          static::setFlashSalesStatus($Qspecials->valueInt('specials_id'), 1);
        } while ($Qspecials->fetch());
      }
    }

// Auto expire products on special
    public static function expireFlashSales()
    {
      $CLICSHOPPING_Db = Registry::get('Db');
      ...
      if ($Qspecials->fetch() !== false) {
        do {
          static::setFlashSalesStatus($Qspecials->valueInt('specials_id'), 0);
        } while ($Qspecials->fetch());
      }
    }
...
// create column list
      $define_list = [
        'MODULE_PRODUCTS_FLASH_SALES_LIST_DATE_ADDED' => MODULE_PRODUCTS_FLASH_SALES_LIST_DATE_ADDED,
        'MODULE_PRODUCTS_FLASH_SALES_LIST_PRICE' => MODULE_PRODUCTS_FLASH_SALES_LIST_PRICE,
        'MODULE_PRODUCTS_FLASH_SALES_LIST_MODEL' => MODULE_PRODUCTS_FLASH_SALES_LIST_MODEL,
        'MODULE_PRODUCTS_FLASH_SALES_LIST_WEIGHT' => MODULE_PRODUCTS_FLASH_SALES_LIST_WEIGHT,
        'MODULE_PRODUCTS_FLASH_SALES_LIST_QUANTITY' => MODULE_PRODUCTS_FLASH_SALES_LIST_QUANTITY,
      ];
    ...
    private static function Listing()
    {
      ...
      if ((!isset($_GET['sort'])) || (!preg_match('/^[1-8][ad]$/', $_GET['sort'])) || (substr($_GET['sort'], 0, 1) > count($count_column))) {
        for ($i = 0, $n = count($count_column); $i < $n; $i++) {
          if ($count_column[$i] == 'MODULE_PRODUCTS_FLASH_SALES_LIST_DATE_ADDED') {
            $_GET['sort'] = $i + 1 . 'a';
    ...
    public static function getListing()
    {
      ...
      if ($CLICSHOPPING_Customer->getCustomersGroupID() != 0) {
        $QlistingFlashSales = $CLICSHOPPING_Db->prepare($Qlisting);
        $QlistingFlashSales->bindInt(':customers_group_id', (int)$CLICSHOPPING_Customer->getCustomersGroupID());
      } else {
        $QlistingFlashSales = $CLICSHOPPING_Db->prepare($Qlisting);
      }

      return $QlistingFlashSales;
    }
  }
